...
def run_analysis():
    ...
    try:
        ...
        print(f"✅ 抓取今日沽空數據")
    except: df_short_today = pd.DataFrame(columns=['股票代碼', 'short_today'])
    ...
    for i, (_, row) in enumerate(df_f.iterrows()):
        ...
        final_results.append({
            "code": row['股票代碼'], "name": row[n_col], "inflow": round(inflow, 2),
            "short_today": round(curr_s, 2), "short_avg": round(avg_s, 2),
            "insight": insight, "rank_change": 0
        })
    ...
    # 🚩 6. Telegram 最終推送 (含日誌診斷)
    if len(TELEGRAM_TOKEN) > 5 and len(CHAT_ID) > 5:
        try:
            api_url = f"https://api.telegram.org{TELEGRAM_TOKEN}/sendMessage"
            msg = f"📊 *港股 AI 看板更新成功*\n時間: {output['update_time']}\n榜首: {final_results[0]['name']}"
            res = requests.post(api_url, data={"chat_id": CHAT_ID, "text": msg, "parse_mode": "Markdown"}, timeout=15)
            print(f"📬 Telegram 回應碼: {res.status_code} | 回傳訊息: {res.text}")
        except Exception as e:
            print(f"❌ 推送失敗: {e}")
    else:
        print("⚠️ 未偵測到有效的 Telegram Secrets，請檢查 GitHub 設定。")
