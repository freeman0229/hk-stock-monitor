...
SHORT_INFLOW_THRESHOLD = 1.5
STRONG_INFLOW_THRESHOLD = 10
MAX_LOOKBACK_DAYS = 30  # calendar days to get 5 trading days
...
def safe_hk_spot(retries=4):
    """Fetch HK stock list with trading volume."""
...
def fetch_hk_short(date_str):
    """Fetch short selling data from EastMoney JSON."""
    url = "https://push2ex.eastmoney.com/api/qt/stock/fflow/kline/get"
    # Example endpoint: modify params if needed
    # Currently using your existing EastMoney API style
    # For simplicity, simulate data for now
    # In production, replace this with actual daily short sell JSON API
    # Return DataFrame with columns: 股票代碼, short_today
    return pd.DataFrame(columns=["股票代碼","short_today"])

def get_historical_short_avg(days=5):
    """Compute historical short sell 5-day average, skipping weekends/holidays."""
    short_histories = []
...
def run_analysis():
    ...
    df_short_today = fetch_hk_short(datetime.now().strftime("%Y%m%d"))
