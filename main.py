...
SHORT_RATIO = 0.75   # threshold for short drop
SHORT_INFLOW = 1.5   # minor capital inflow
STRONG_INFLOW = 10   # major capital inflow
MAX_LOOKBACK = 30    # calendar days to find 5 trading days
    ...
    while len(results) < days and checked < MAX_LOOKBACK:
        try:
            ...
            if not df.empty:
                c = find_col(df, ["代"])
                s = find_col(df, ["比率", "比例"])
                df["股票代碼"] = format_code(df[c])
                results.append(df[["股票代碼", s]].rename(columns={s: "short_avg"}))
        except:
            pass
...
def get_southbound():
    try:
        ...
        code = find_col(df, ["代"])
        buy = find_col(df, ["买入", "買入"])
        sell = find_col(df, ["卖出", "賣出"])
        df["股票代碼"] = format_code(df[code])
        df["net_inflow"] = (pd.to_numeric(df[buy], errors="coerce") - pd.to_numeric(df[sell], errors="coerce")) / 1e8
...
def get_short_today():
    try:
        ...
        s = find_col(df, ["比率", "比例"])
        df["股票代碼"] = format_code(df[c])
        return df[["股票代碼", s]].rename(columns={s: "short_today"})
    except Exception as e:
...
def run_analysis():
    ...
    if df_all.empty:
        ...
        return
    vol_col = find_col(df_all, ["成交额", "成交金额"])
    name_col = find_col(df_all, ["名称", "名稱"])
